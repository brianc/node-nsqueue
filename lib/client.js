...
Client.prototype.identify = function(options, cb) {
  ...
  var con = this.con
  return maybeCallback(con, function(err, res) {
    if(err) return cb(err);
    try {
      var data = JSON.parse(res)
    } catch(e) {}
    if(!tls) return cb(null, data);
    //there are tls options. initiate upgrade
    con.upgradeToTLS(tls, function(err) {
      if(err) return cb(err);

      //nsqd responds with OK
      maybeCallback(con, function(err, res) {
        if(err) return cb(err);
        cb(null, data)
      })
    })
