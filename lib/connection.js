...
Connection.prototype.upgradeToTLS = function(options, callback) {
  var tls = require('tls')
  console.log('upgrading')

  this.stream.removeAllListeners('data')

  options.socket = this.stream
  var self = this
  var tlsstream = tls.connect(options, function(err) {
    if(err) return callback(error)
    console.log('upgraded')
    //self.stream.removeAllListeners()
    self.stream = tlsstream
    tlsstream.on('data', self.parse.bind(self))
    callback()
  })
  tlsstream.on('error', function(e) {
    self.emit(e)
  })
}
